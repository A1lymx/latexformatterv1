...
class Command {
  constructor(name, optionalArgument = null, argument = null) {
    this.name = name;
    this.optionalArgument = optionalArgument;
    this.argument = argument; // Only supports a single required argument
  }
  toString() {
    return `Command(name=${JSON.stringify(this.name)}, optionalArgument=${JSON.stringify(this.optionalArgument)}, argument=${JSON.stringify(this.argument)})`;
  }
}
...
class Parser {
  ...
  parse() {
    ...
    while (this.currentToken.type !== TOKEN_EOF) {
      iteration++;
      console.log(`[DEBUG][Parser] parse() loop iteration ${iteration}, current token: ${this.currentToken.toString()}`);
      ...
      if (node !== null) {
        ...
        if (this.currentToken.type !== TOKEN_EOF) {
          console.log(`[DEBUG][Parser] parseElement returned null, force consuming token: ${this.currentToken.toString()}`);
  ...
  parseCommand() {
    const token = this.currentToken;
    console.log(`[DEBUG][Parser] Parsing command: ${token.toString()}`);
    this.eat(TOKEN_COMMAND);
    let optionalArgument = null;
    let argument = null;

    // Check for an optional argument in square brackets
    if (this.currentToken.type === TOKEN_LBRACKET) {
      this.eat(TOKEN_LBRACKET);
      let optContent = "";
      while (this.currentToken.type !== TOKEN_RBRACKET) {
        console.log(`[DEBUG][Parser] In optional argument, current token: ${this.currentToken.toString()}`);
    ...
    // Check for a required argument in braces
    if (this.currentToken.type === TOKEN_LBRACE) {
      this.eat(TOKEN_LBRACE);
      let argContent = "";
      while (this.currentToken.type !== TOKEN_RBRACE) {
        console.log(`[DEBUG][Parser] In required argument, current token: ${this.currentToken.toString()}`);
        argContent += this.currentToken.value;
        this.eat(this.currentToken.type);
      }
    ...
    return new Command(token.value, optionalArgument, argument);
  }
  ...
  parseEnvironment() {
    ...
    while (true) {
      iteration++;
      console.log(`[DEBUG][Parser] Environment content loop iteration ${iteration}, current token: ${this.currentToken.toString()}`);
      ...
      if (node !== null) {
        ...
        if (this.currentToken.type !== TOKEN_EOF) {
          console.log(`[DEBUG][Parser] In environment, parseElement returned null, force consuming token: ${this.currentToken.toString()}`);
          this.eat(this.currentToken.type);
        }
      }
    }
    ...
    let endEnvName = "";
    while (this.currentToken.type !== TOKEN_RBRACE) {
      console.log(`[DEBUG][Parser] Parsing \\end environment name, current token: ${this.currentToken.toString()}`);
